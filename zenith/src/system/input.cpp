...
auto Input::init() -> Result<void, String>
{
    ...
    for (auto key : meta::enum_values<Key>())
        _key_states.insert({ key, false });

    for (auto button : meta::enum_values<MouseButton>())
...
ZTH_DEFINE_REFLECTED_ENUM(zth::Key);
ZTH_DEFINE_REFLECTED_ENUM(zth::MouseButton);
