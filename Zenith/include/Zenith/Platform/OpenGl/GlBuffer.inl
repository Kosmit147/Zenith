...
auto GlBuffer::buffer_sub_data(std::ranges::contiguous_range auto&& data, GLintptr offset) const -> void
{
    using T = std::ranges::range_value_t<decltype(data)>;
...
auto GlBuffer::buffer_sub_data(auto&& object, GLintptr offset) const -> void
{
    using T = decltype(object);
    auto data_size_bytes = static_cast<GLsizeiptr>(sizeof(T));

    ZTH_ASSERT(data_size_bytes + offset <= size_bytes());
    glNamedBufferSubData(native_handle(), offset, data_size_bytes, &object);
}
